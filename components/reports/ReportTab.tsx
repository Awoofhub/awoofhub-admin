...
export default function ReportTabs({ tabs, activeTab, onChange }: Props) {
    ...
    return (
        <div className="bg-[#F3F3F5] p-1.5 rounded-xl flex items-center gap-2 mb-6">
            {tabs.map((tab) => (
                <button
                    key={tab.label}
                    onClick={() => onChange(tab.value)}
                    className={`px-3 md:px-4 py-2.5 md:py-2 cursor-pointer rounded-2xl text-[10px] md:text-xs lg:text-sm font-semibold whitespace-nowrap transition-colors
