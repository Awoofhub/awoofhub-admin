...
import { useOfferInsights } from '@/features/offer/useOfferInsights';
...
function buildWeeksInMonth(
    offers: { createdAt: string }[],
    monthIndex: number,
    year: number,
    monthLabel: string, 
) {
    ...
    return [...weekData, { label: monthLabel, offers: monthTotal }];
}
...
export default function OffersPerWeekChart() {
    ...
    const monthLabel = selectedMonth === 'this-month' ? 'This month' : MONTH_NAMES[monthIndex];

    const { offers, isLoading } = useOfferInsights(monthIndex, year);
    const chartData = useMemo(
        () => buildWeeksInMonth(offers, monthIndex, year, monthLabel),
        [offers, monthIndex, year, monthLabel]
    );
