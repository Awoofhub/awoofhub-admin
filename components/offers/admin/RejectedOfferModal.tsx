// RejectedOfferModal.tsx
// Full-screen overlay modal shown when the admin clicks the eye icon on a
// rejected offer in the All Offers table. Displays the offer preview and
// provides a View-more action.

"use client";

import Image from "next/image";
import { useState } from "react";
import { X, MapPin, Globe, Link as LinkIcon, XCircle } from "lucide-react";
...
interface RejectedOfferModalProps {
  offer: Offer;
  onClose: () => void;
...
export default function RejectedOfferModal({
  offer,
  onClose,
  onViewMore,
}: RejectedOfferModalProps) {
      ...
      <div className="relative w-full max-w-[95vw] sm:max-w-[480px] md:max-w-[600px] lg:max-w-[782px] animate-in fade-in zoom-in-95">
        ...
        <div className="p-4 sm:p-5 md:p-6 lg:p-8 space-y-3 sm:space-y-4 md:space-y-5 lg:space-y-6">
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            <span className="inline-flex items-center gap-1.5 shrink-0 rounded-full bg-red-50 px-3 py-1 text-xs font-medium text-red-500">
          ...
          <div className="flex pt-1">
            <button
              ...
              className="w-full border-2 border-orange-400 text-orange-500 hover:bg-orange-50 font-bold py-1 rounded-lg transition-colors text-sm"
            >
              View more
            </button>
          </div>
