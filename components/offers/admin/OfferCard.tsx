      ...
      <div className="flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <span className="font-semibold text-gray-700 text-sm">@{offer.business?.name?.replace(/\s+/g, '').toLowerCase() || 'awoofqueen'}</span>
        ...
        {/* Title */}
        <h3 className="text-[17px] leading-tight sm:text-[19px] font-bold text-slate-900 mb-3">{offer.title}</h3>

        {/* Tags */}
        <div className="flex flex-wrap gap-2 mb-3">
          <span className="border border-gray-200 text-gray-600 text-xs px-3 py-1 rounded-full font-medium">{dealType}</span>
          <span className="border border-gray-200 text-gray-600 text-xs px-3 py-1 rounded-full font-medium">{categoryName}</span>
          <span className="border border-gray-200 text-gray-600 text-xs px-3 py-1 rounded-full font-medium">{brandName}</span>
        </div>

        {/* Location & Link */}
        <div className="flex flex-col gap-1.5 mb-4">
          <div className="flex items-center gap-1.5 text-gray-500 text-sm">
            {isOnline ? <Globe className="w-4 h-4 text-gray-400" /> : <MapPin className="w-4 h-4 text-gray-400" />}
          ...
          <div className="flex items-center gap-1.5 text-orange-500 text-sm">
            ...
            <a href={offer.externalLink || '#'} target="_blank" rel="noopener noreferrer" className="hover:underline line-clamp-1">{offer.externalLink }</a>
          </div>
        </div>

        {/* Expiry */}
        <div className="flex items-center justify-between mb-4">
          <span className="text-gray-500 text-[13px] font-medium">Expires: <span className="text-slate-800 font-bold">{format(new Date(offer.endDate), 'd/M/yyyy')}</span></span>
          <span className="bg-red-50 text-red-500 text-xs px-4 py-1 rounded-full font-bold capitalize">{dealType}</span>
        </div>

        {/* Details */}
        <div className="mb-5 flex-1">
          <span className="font-bold text-[11px] text-slate-800 mb-1 block">Details</span>
          <p className={isExpanded ? 'text-gray-500 text-[13px] leading-relaxed' : 'text-gray-500 text-[13px] line-clamp-2 md:line-clamp-1 leading-relaxed'}>
            {offer.description}
          </p>
          {isExpanded ? (
            <button
