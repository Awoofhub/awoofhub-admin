...
import { Check, X, Edit2, History, Clock, ChevronDown } from "lucide-react";
...
export default function OfferDetailPage() {
  ...
  >(null);
  const [isModerationDropdownOpen, setIsModerationDropdownOpen] =
    useState(false);
      ...
      <section className="pt-14 px-6 w-full h-[90dvh] md:h-[88dvh]">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 max-w-lg mx-auto">
          ...
          <p className="text-red-600">
            {error?.message ?? "Failed to load offer details."}
          </p>
          <button
            onClick={() => router.back()}
      ...
      <div className="py-4 sm:py-6 md:py-8 px-3 sm:px-4 md:px-6 mx-auto w-full max-w-360 flex flex-col h-full">
        <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 shrink-0 relative">
          <div className="flex-1">
            <button
              onClick={() => router.back()}
              className="text-primary hover:text-orange-700 font-semibold mb-3 flex items-center gap-2 text-sm sm:text-base"
            >
              ← Back to Offers
            </button>
          ...
          <div className="flex items-center gap-3">
            <div
              className={`px-3 sm:px-4 py-2 rounded-lg font-bold text-xs sm:text-sm whitespace-nowrap capitalize ${statusConfig[currentStatus] || "bg-gray-50 text-gray-500 border border-gray-200"}`}
            >
              {currentStatus}
            </div>
            ...
            <div className="relative" ref={dropdownRef}>
              ...
              >
                ...
                <ChevronDown
                  className={`w-4 h-4 transition-transform ${isModerationDropdownOpen ? "rotate-180" : ""}`}
                />
              </button>
              ...
              <div className="bg-gray-50 rounded-lg p-4 sm:p-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
                  <Rating
                    ...
                    readOnly
                    size="large"
                    sx={{ "& .MuiRating-iconFilled": { color: "#FFC000" } }}
                  />
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 sm:p-6">
                ...
                {isLoadingHistory ? (
                    ...
                    {moderationHistory.map((mod: any, i: number) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-1">
                          <span
                            className={`font-bold capitalize text-xs ${mod.actionType === "block" ? "text-red-600" : mod.actionType === "suspend" ? "text-orange-600" : "text-green-600"}`}
                          >
                            Action: {mod.actionType}
                          </span>
                          <span className="text-xs text-gray-400">
                            {formatDateTime(mod.createdAt)}
                          </span>
                        </div>
                  ...
                  <p className="text-sm text-blue-600 italic">
                    No previous moderation actions or notes found.
                  </p>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-lg p-6 sm:p-8 max-w-md w-full shadow-2xl">
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  disabled={
                    isModerating ||
                    (!adminNote.trim() && moderationAction !== "approved")
                  }
